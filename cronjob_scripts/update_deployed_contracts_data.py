...
from sqlalchemy import Column, Integer, String, Date, Float, create_engine, Boolean
import time
from datetime import datetime
import json
import os

# TODO possible optimization: pass also the already created contract from the caller script
def update_contract_info(contract_addr):
    ...
    ranking = Contract.functions.ranking().call()
    ...
    try:
        ...
        row = Contracts_info(contract_address=contract_addr, contract_start=contract_start, contract_end=contract_end, time_item_delivered=time_item_delivered,\
                                has_buyer_paid=has_buyer_paid, paid_ammount=paid_ammount, refounded=refounded, ranking=ranking)
        session.add(row)
        session.commit()

def update_contract_status(contract_addr, status):
    """
    Change the status of the contract in the database to 2 or 3 based if the notification was a delivery or a refound
    """
    # i know that the contract already exists if i call this method, so i just need to query the correct line and change
    # the value form the current one to the passed one

    Base = declarative_base()

    class Contracts_deployed(Base):
        __tablename__ = 'contracts_deployed'
        id = Column(Integer, primary_key=True)
        request_id = Column(String)
        contract_creator_user = Column(String)
        contract_address = Column(String)
        contract_name = Column(String)
        deployer = Column(String)
        seller = Column(String)
        buyer = Column(String)
        oracle = Column(String)
        contract_time = Column(Integer)
        contract_delivery_eta = Column(Integer)
    ...
    row = session.query(Contracts_deployed).filter_by(contract_address=contract_addr).first()
    row.status = status
    session.commit()
...
def update_contract_info_cron(contract_addr):
    ...
    # TODO checks that the contract status is 1, else it removes the cronjob
    current_contract = session.query(Contracts_deployed).filter_by(contract_address = contract_addr).first()
    if current_contract.status != 1:
        # TODO remove the cronjob
        command_to_delete = f'/home/andrea/anaconda3/envs/vyper/bin/python /home/andrea/Desktop/paywac_website/cronjob_scripts/cron_update_info_paywac.py {contract_addr} > /home/andrea/Desktop/paywac_website/logs/cron.log 2>&1'
    ...
    # TODO check for the status of variables refounded, time_item_delivered
    ...
    if refounded == True or refounded == 'true' or refounded == 'True':
        ...
        session.commit()
    elif time_item_delivered_unix != 0:
