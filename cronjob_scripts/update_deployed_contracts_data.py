...
# try to use the same table
def update_contract_erc20_info_cron(contract_addr):
    try:
        Base = declarative_base()
        ...
        time_item_delivered = datetime.fromtimestamp(Contract.functions.time_item_delivered().call())
        time_item_delivered_unix = Contract.functions.time_item_delivered().call()
        has_buyer_paid = Contract.functions.is_paid().call()
        # converting prices, this will have to be modified once we use it with USDT
        item_price = Contract.functions.item_price().call() / 1000
        shipping_price = Contract.functions.shipping_price().call() / 1000
        paid_ammount = Contract.functions.contract_balance().call() / 1000
        refounded = Contract.functions.refounded().call()
        latest_update = datetime.now()
        ...
        if refounded == True or refounded == 'true' or refounded == 'True':
            ...
            session.commit()
            #remove the cronjob
            command_to_delete = f'/home/andrea/anaconda3/envs/vyper/bin/python /home/andrea/Desktop/paywac_website_v02/cronjob_scripts/cron_update_info_paywac_erc20.py {contract_addr} >> /home/andrea/Desktop/paywac_website_v02/logs/cron.log_{contract_addr} 2>&1'
            os.system(f"crontab -u andrea -l | grep -v '{command_to_delete}'  | crontab -u andrea -")
            print(str(datetime.now())+' -- Contract not respected, eth refounded')
